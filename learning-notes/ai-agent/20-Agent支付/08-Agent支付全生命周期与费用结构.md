...
| Network Token (EvoNet) | 1.5%-3.5% + 网关费 | ~$1 | T+1 到 T+3 | ✅ 卡组织争议 |
...
```text
                    x402        MPP         ACP         MC Agent Pay   Network Token   Stripe Issuing   AP2
                    ────        ───         ───         ────────────   ─────────────   ──────────────   ───
发起支付前准备
  需要钱包？         ✅ 需要     ✅ 需要     ❌ 不需要    ❌ 不需要       ❌ 不需要        ❌ 不需要        ❌ 不需要
  需要 KYC？         ❌ 不需要   ❌ 不需要    ❌ 不需要    ✅ KYA 验证     ❌ 不需要        ✅ KYB 审核      ⚠️ 取决于底层
  需要预存资金？     ✅ USDC     ✅ 预授权    ❌ 不需要    ❌ 不需要       ❌ 不需要        ✅ Issuing 余额  ⚠️ 取决于底层
  Agent 身份验证？   ❌ 无       ❌ 无        ❌ 无        ✅ KYA 4级      ❌ 无            ⚠️ Cardholder   ✅ Mandate 签名

支付过程
  交互次数（最少）   2 次        3 次        4+ 次       3+ 次          3 次             2 次             6+ 次
  需要用户参与？     ❌ 不需要   ❌ 不需要    ✅ 需要确认  ⚠️ 首次授权    ❌ 不需要        ❌ 不需要        ✅ 签名确认
  支持离线/委托？    ✅ 完全自主 ✅ 会话内自主 ❌ 需要用户  ✅ Recurring    ✅ 令牌可复用    ✅ 完全自主      ✅ Intent 委托

结算
  结算速度           秒级        会话结束时   T+2         T+1 到 T+3     T+1 到 T+3      T+2              取决于底层
  结算确定性         ✅ 链上确认 ✅ 链上确认  ⚠️ 可退款   ⚠️ 可争议      ⚠️ 可争议       ⚠️ 可争议       取决于底层

安全与控制
  限额控制           ❌ 无       ✅ 会话预算  ✅ SPT 限额  ✅ Token 限额   ✅ 策略引擎      ✅ 精细控制      ✅ Mandate 约束
  商家限制           ❌ 无       ❌ 无        ✅ SPT 限商家 ✅ MCC 限制    ✅ MCC 限制      ✅ MCC 限制      ✅ Mandate 约束
  实时审批           ❌ 无       ❌ 无        ❌ 无        ❌ 无           ❌ 无            ✅ Webhook       ❌ 无
  审计记录           ✅ 链上     ✅ 链上      ✅ Stripe    ✅ 卡网络       ✅ EvoNet        ✅ Stripe        ✅ Mandate 链
```
...
```text
                    x402        MPP         ACP         MC Agent Pay   Network Token   Stripe Issuing   AP2
                    ────        ───         ───         ────────────   ─────────────   ──────────────   ───
余额不足             自行检测    会话创建时   SPT 创建时   授权时拒绝     授权时拒绝       授权时拒绝       取决于底层
网络中断             可能双付    会话保护     SPT 幂等     卡网络幂等     GET 查询结果     卡网络幂等       Mandate 可重放
服务方宕机           钱可能丢    会话超时释放  SPT 过期     授权未捕获     授权未捕获       授权未捕获       无资金风险
支付后未交付         无保护      有限保护     完整保护     完整保护       完整保护         完整保护         取决于底层
```
...
```text
                    x402        MPP         ACP         MC Agent Pay   Network Token   Stripe Issuing   AP2
                    ────        ───         ───         ────────────   ─────────────   ──────────────   ───
主动退款             ❌ 不可逆   ⚠️ 会话内   ✅ Stripe    ✅ 商家退款    ✅ EvoNet 退款   ✅ 商家退款      取决于底层
争议/Chargeback      ❌ 无       ⚠️ 有限     ✅ 卡组织    ✅ 卡组织      ✅ 卡组织        ✅ 卡组织        取决于底层
欺诈保护             ❌ 无       ⚠️ 有限     ✅ Reg Z     ✅ 卡组织规则  ✅ 卡组织规则    ✅ 卡组织规则    ✅ Mandate 审计
争议解决时间         N/A         N/A         30-90 天     30-90 天       30-90 天         30-90 天         取决于底层
消费者保护法适用     ❌ 不适用   ❌ 不适用    ✅ 适用      ✅ 适用        ✅ 适用          ✅ 适用          取决于底层
```
