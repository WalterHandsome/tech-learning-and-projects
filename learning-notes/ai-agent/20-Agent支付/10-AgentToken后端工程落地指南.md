...
CREATE TABLE tokens (
    ...
    status          VARCHAR(20) NOT NULL DEFAULT 'open'
                    CHECK (status IN ('open', 'exhausted', 'closed', 'expired')),
...
CREATE TABLE payment_transactions (
    ...
    currency        VARCHAR(3) NOT NULL DEFAULT 'USD',
    -- 外部网关信息
    gateway         VARCHAR(20) NOT NULL,  -- 'stripe' | 'marqeta' | 'visa_dps' | 'x402'
...
统一接口内部路由：
  token_type == "vcn"           → VCNGateway（对接 Marqeta / Stripe Issuing）
  token_type == "network_token" → NetworkTokenGateway（对接 Visa VTS / MC MDES）
...
### 6.2 VCN 对接（以 Stripe Issuing 为例）

```text
Stripe Issuing 是最容易上手的虚拟卡发卡平台。
其他选择：Marqeta（更灵活）、Lithic（更轻量）、Adyen Issuing。

对接流程：
  1. 创建 Cardholder（对应你的 Member）
  2. 创建 Card（虚拟卡，对应你的 Token）
  3. 设置 Spending Controls（对应你的 Policy）
  4. 监听 Webhook（交易通知、授权请求）
...
### 6.3 Network Token 对接

```text
Network Token 比 VCN 复杂，因为要直接对接卡组织（Visa/Mastercard）。
大多数团队不会直接对接卡组织，而是通过中间商：
...
通过中间商（推荐，适合创业公司）：
  → Stripe（自动处理 Network Tokenization）
  → Adyen（支持 Network Token 透传）
  → Checkout.com

对接要点：
  1. 请求卡组织为真实卡号生成 Network Token
  2. 每次交易时请求动态 Cryptogram
  3. 将 Token + Cryptogram 发送给收单行
  4. 卡过期/换卡时，卡组织自动更新映射（你不用管）
```
...
沙箱环境 vs 生产环境：
  ...
  ├─ 网关层面：沙箱用 Stripe 的测试模式，生产用 Stripe 的生产模式
