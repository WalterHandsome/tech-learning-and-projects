...
本目录存放 AI Agent 领域的每日信息简报，由 Kiro Hook 自动采集生成。
...
**关注领域**：
- AI Agent 框架（LangGraph、CrewAI、OpenAI SDK、ADK、Strands）
- Agent 协议（MCP、A2A、ACP）
- RAG 与向量数据库
- Coding Agent（Claude Code、Cursor、Kiro）
- Agent 安全与治理
- Context Engineering
...
在 Kiro 中点击 Agent Hooks → `📰 AI Agent 每日简报` → 手动触发即可生成当日简报。
