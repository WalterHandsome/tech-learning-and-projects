...
# - 不检查代码块语言标注（ASCII 图表/流程图不需要语言标记，强制标注会破坏格式）
# - 不检查标题层级跳跃（shell 脚本无法可靠区分代码块内外的 # 符号）
...
find "$DOC_DIR" -name "*.md" -type f | while read -r file; do
    ...
    # 检查纯文本引用: 详见 → xxx.md（未使用 Markdown 链接格式）
    grep -n '详见.*→' "$file" 2>/dev/null | grep -v '\[.*\](.*)' | grep '\.md' | while IFS= read -r match; do
...
find "$DOC_DIR" -name "*.md" -type f ! -name "README.md" ! -name ".audit-progress.md" ! -name ".update-log.md" | while read -r file; do
...
TOTAL=$(find "$DOC_DIR" -name "*.md" -type f ! -name "README.md" ! -name ".audit-progress.md" ! -name ".update-log.md" | wc -l | tr -d ' ')
